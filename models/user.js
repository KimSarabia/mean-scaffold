...
const JWT_SECRET = 'Told em I finished school, and I started my own business';
...
userSchema.statics.authMiddleware = function(req, res, next) {
  var token = req.cookies.kimcookie;
  try {
    var payload = jwt.decode(token, JWT_SECRET);
  } catch(err) {
    return res.clearCookie('kimcookie').status(401).send();
  }
  // we have a valid token

  User.findById(payload.userId).select({password: 0}).exec(function(err, user) {
    if(err || !user) {
      return res.clearCookie('kimcookie').status(401).send(err);
    }
    // the user exists!
